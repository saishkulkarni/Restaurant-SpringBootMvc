...
<body>
	<h1>Edit Item</h1>
	<div class="container">
		<form action="/hotel/edit-item" method="post"
			enctype="multipart/form-data">
			<input name="id" value="${item.getId()}" hidden="hidden">
			<fieldset>
				<legend>Item Details</legend>
				<label for="name">Name:</label> <input type="text" id="name"
					name="name" value="${item.getName()}" required> <label
					for="price">Price:</label> <input type="number" id="price"
					name="price" value="${item.getPrice()}" required> <label
					for="image">Image:</label> <img class="product-image" height="150px"
				...
				<input type="file" id="image" name="image"> <label
					...
					name="stock" value="${item.getStock()}" required> <label
					for="description">Description:</label> <input type="text"
					id="description" name="description"
					value="${item.getDescription()}" required>
			</fieldset>
